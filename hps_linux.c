...
// It was found that the data captured by signal tap logic analyzer is
// delayed by an arbitrary DCO clock cycles, therefore data needs to
// be shifted by some number given in the read_adc_val because
// this might not be the case with different FPGA implementation!
// check the delay using user input pattern
...
void create_measurement_folder(char * foldertype) {
	...
	char command[60];
	sprintf(foldername, "%s_%04d_%02d_%02d_%02d_%02d_%02d", foldertype,
...
unsigned int write_adc_spi(unsigned int comm) {
	...
	while (!(alt_read_word(h2p_adcspi_addr + SPI_STATUS_offst)
			& (1 << status_TRDY_bit)))
		;
...
unsigned int write_ad9276_spi(unsigned char rw, unsigned int addr,
		unsigned int val) {
...
void write_beamformer_spi(unsigned char spi_reg_length, unsigned char read,
		unsigned char spi_addr, unsigned long spi_data_out,
...
void init_adc() {
	...
	write_ad9276_spi(AD9276_SPI_WR, AD9276_FLEX_GAIN_REG,
			AD9276_PGA_GAIN_21dB_VAL << AD9276_PGA_GAIN_SHFT
					| AD9276_LNA_GAIN_15dB_VAL << AD9276_LNA_GAIN_SHFT); // set PGA Gain to 21 dB, LNA Gain to 15.6 dB
	write_ad9276_spi(AD9276_SPI_WR, AD9276_OUT_ADJ_REG,
			AD9276_OUT_ADJ_TERM_200OHM_VAL << AD9276_OUT_ADJ_TERM_SHFT); // set output driver to 100 ohms
	write_ad9276_spi(AD9276_SPI_WR, AD9276_OUT_PHS_REG,
			AD9276_OUT_PHS_060DEG_VAL); // set phase to 000 degrees
	write_ad9276_spi(AD9276_SPI_WR, AD9276_DEV_UPDT_REG, AD9276_SW_TRF_MSK); // update the device

	// filter setup
	write_ad9276_spi(AD9276_SPI_WR, AD9276_FLEX_FILT_REG,
			AD9276_FLEX_FILT_HPF_04PCTG_FLP_VAL); // set high-pass filter
	write_ad9276_spi(AD9276_SPI_WR, AD9276_DEV_UPDT_REG, AD9276_SW_TRF_MSK); // update the device

	write_ad9276_spi(AD9276_SPI_WR, AD9276_DEV_IDX_1_REG,
			AD9276_DCO_CMD_EN_MSK | AD9276_FCO_CMD_EN_MSK); // select DCO and FCO
	write_ad9276_spi(AD9276_SPI_WR, AD9276_DEV_IDX_2_REG, 0x00);
	write_ad9276_spi(AD9276_SPI_WR, AD9276_OUT_MODE_REG,
			AD9276_OUT_MODE_INVERT_EN_MSK); // invert DCO and FCO
	write_ad9276_spi(AD9276_SPI_WR, AD9276_DEV_UPDT_REG, AD9276_SW_TRF_MSK); // update the device

	write_ad9276_spi(AD9276_SPI_WR, AD9276_DEV_IDX_1_REG,
			AD9276_CH_A_CMD_EN_MSK | AD9276_CH_B_CMD_EN_MSK
					| AD9276_CH_C_CMD_EN_MSK | AD9276_CH_D_CMD_EN_MSK); // select channel A-D
	write_ad9276_spi(AD9276_SPI_WR, AD9276_DEV_IDX_2_REG,
			AD9276_CH_E_CMD_EN_MSK | AD9276_CH_F_CMD_EN_MSK
					| AD9276_CH_G_CMD_EN_MSK | AD9276_CH_H_CMD_EN_MSK); // select channel E-H
	write_ad9276_spi(AD9276_SPI_WR, AD9276_OUT_MODE_REG,
			AD9276_OUT_MODE_INVERT_EN_MSK); // invert all selected channel
	write_ad9276_spi(AD9276_SPI_WR, AD9276_TESTIO_REG, 0x00); // reset the testio

	write_ad9276_spi(AD9276_SPI_WR, AD9276_TESTIO_REG, AD9276_OUT_TEST_OFF_VAL); // select default test pattern
	write_ad9276_spi(AD9276_SPI_WR, AD9276_DEV_UPDT_REG, AD9276_SW_TRF_MSK); // update the device
...
void select_pattern(uint8_t pattern_val) {
	// select test pattern for driving the ADC output
	write_ad9276_spi(AD9276_SPI_WR, AD9276_TESTIO_REG, pattern_val); // select testpattern
	write_ad9276_spi(AD9276_SPI_WR, AD9276_DEV_UPDT_REG, AD9276_SW_TRF_MSK); // update the device
}

void set_pattern(uint16_t pattern1, uint16_t pattern2) {
	write_ad9276_spi(AD9276_SPI_WR, AD9276_TESTIO_REG,
			AD9276_OUT_TEST_USR_INPUT_VAL); // user input test

	// write pattern 1
	write_ad9276_spi(AD9276_SPI_WR, AD9276_USR_PATT1_LSB_REG, pattern1 & 0xFF); // user input values
	write_ad9276_spi(AD9276_SPI_WR, AD9276_USR_PATT1_MSB_REG,
			(pattern1 >> 8) & 0xFF); // user input values

	// write pattern 2
	write_ad9276_spi(AD9276_SPI_WR, AD9276_USR_PATT2_LSB_REG, pattern2 & 0xFF); // user input values
	write_ad9276_spi(AD9276_SPI_WR, AD9276_USR_PATT2_MSB_REG,
			(pattern2 >> 8) & 0xFF); // user input values
...
void init_beamformer() {
	...
	write_beamformer_spi(REG_1A_LENGTH, LM86570_SPI_WR, 0x1A, 0x4600, &spi_in0,
			&spi_in1, &spi_in2);
	...
	write_beamformer_spi(4, LM86570_SPI_WR, 0x18, 0x0005, &spi_in0, &spi_in1,
			&spi_in2); // write all p registers
	write_beamformer_spi(4, LM86570_SPI_WR, 0x19, 0x000A, &spi_in0, &spi_in1,
			&spi_in2); // write all n registers

	write_beamformer_spi(4, REG_1BA_LENGTH, 0x1B, 0x0000, &spi_in0, &spi_in1,
			&spi_in2);
...
void read_adc_val(void *channel_csr_addr, void *channel_data_addr,
	...
	// WARNING: this module does data conversion to compensate for error in counting bit
	// when the data is captured. For example, instead of having bit-0 placed on the LSB,
	// the bit-0 is captured and placed on LSB+4 bit, which is wrong.
	...
	fifo_mem_level = alt_read_word(
			channel_csr_addr + ALTERA_AVALON_FIFO_LEVEL_REG); // the fill level of FIFO memory
	...
	for (i = 0; fifo_mem_level > 0; i++) {
		...
		if (fifo_mem_level == 0) {
			fifo_mem_level = alt_read_word(
					channel_csr_addr + ALTERA_AVALON_FIFO_LEVEL_REG);
		}
	}
	usleep(100);
...
void store_data(unsigned int * adc_data,
...
void store_data_2d(unsigned int * adc_data,
		unsigned int data_bank_2d[num_of_channels][num_of_samples],
		unsigned int ch_num, unsigned int num_of_samples) {
	unsigned int ii;
	for (ii = 0; ii < num_of_samples / 2; ii++) {
		data_bank_2d[ch_num][ii * 2] = adc_data[ii] & 0xFFFF; // SHIFTED FROM ORIGINAL
		data_bank_2d[ch_num][ii * 2 + 1] = (adc_data[ii] >> 16) & 0xFFFF; //SHIFTED FROM ORIGINAL
	}
}

void write_data_bank(
		unsigned int data_bank[num_of_switches][num_of_channels][num_of_samples]) {
...
void write_data_2d(unsigned int data_bank_2d[num_of_channels][num_of_samples]) {
	...
	// shift and correct the bit because the bit is shifted in the fpga.
	unsigned int temp_data;
	for (jj = 0; jj < num_of_channels; jj++) {
		for (kk = 0; kk < num_of_samples; kk++) {
			temp_data = (~data_bank_2d[jj][kk]) & 0x3FFF; // invert & null the first two
			data_bank_2d[jj][kk] = (temp_data >> 6)
					+ (((temp_data >> 2) & 0xF) << 8); // the data is 14-bit and the bit-0/bit-1 are the same with bit-12/bit-13.
		}
	}
